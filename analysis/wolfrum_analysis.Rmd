...
colormap <- c(
  ...
  U='#e27268',
  L='#93c8bc',
  'Seurat predicted match to P'='#fc595d',
  'Seurat predicted match to U'='#6789ce',
  'Seurat predicted match to L'='#e295bc',
...
```{r}
merge_wolfrum_180831_clusters <- function(object, cluster_col){
  object@meta.data$filtered_clusters <- unlist(lapply(object@meta.data[cluster_col][[1]], function(x){
  if (x %in% c(5, 14, 23, 11, 10, 24)){
    return(x)
  } else{
    return(NA)
  }
}))
  object@meta.data$filtered_clusters[which(object@meta.data$Dataset == '10x-180831')] <- object@meta.data$branch[which(object@meta.data$Dataset == '10x-180831')]
  return(object)
}
```

Whole dataset with default parameters

```{r, fig.height = 10, fig.width = 12, fig.align = "center"}
#data_180831 <- readRDS('output/seurat_objects/180831/10x-180831-S3')
#wolfrum <- readRDS('output/seurat_objects/wolfrum/wolfrum.compute.seurat_obj.rds')
#wolfrum@meta.data['Dataset'] <- 'Wolfrum'
#data_180831@meta.data['Dataset'] <- '10x-180831'
#data_combined <- merge(data_180831, wolfrum)
#data_combined <- NormalizeData(data_combined)
#data_combined <- FindVariableFeatures(data_combined)
#data_combined <- ScaleData(data_combined, split.by = "Dataset", do.center = FALSE)
#data_combined <- RunPCA(data_combined)
#liger <- RunOptimizeALS(data_combined, k = 10, lambda = 5, split.by = "Dataset")
#liger <- RunQuantileAlignSNF(liger, split.by = "Dataset")
#liger <- RunUMAP(liger, dims = 1:ncol(liger[["iNMF"]]), reduction = "iNMF")
#saveRDS(liger, '/projects/pytrik/sc_adipose/analyze_10x_fluidigm/10x-adipocyte-analysis/output/liger/liger_10x-180831_wolfrum_k10_lambda_5')

load('output/liger/liger_10x-180831_suggestK_suggestLambda')
plot_grid(
  UMAPPlot(liger, group.by='branch'),
  UMAPPlot(liger, group.by='RNA_snn_res.0.8', label=T),
  UMAPPlot(liger, group.by='Dataset'), ncol=2
)
```

Running liger with the parameters from the tutorials doesn't generate good results. Run suggestK and suggestLambda to find the optimal k and lambda values. 

```{r}
#SUGGEST K
#liger_object <- seuratToLiger(liger, combined.seurat=T, meta.var='Dataset', names='use-meta')
...
```{r}
liger_k10_lambda10 <- readRDS('output/liger/liger_10x-180831_wolfrum_k10_lambda_10')
liger_k15_lambda10 <- readRDS('output/liger/liger_10x-180831_wolfrum_k15_lambda_10')
liger_k10_lambda35 <- readRDS('output/liger/liger_10x-180831_wolfrum_k10_lambda_35')
liger_k10_lambda5 <- readRDS('output/liger/liger_10x-180831_wolfrum_k10_lambda_5')
```

```{r, fig.height = 10, fig.width = 12, fig.align = "center"}
liger_k10_lambda10 <- merge_wolfrum_180831_clusters(liger_k10_lambda10, 'RNA_snn_res.0.8')
liger_k15_lambda10 <- merge_wolfrum_180831_clusters(liger_k15_lambda10, 'RNA_snn_res.0.8')
liger_k10_lambda35 <- merge_wolfrum_180831_clusters(liger_k10_lambda35, 'RNA_snn_res.0.8')
liger_k10_lambda5 <- merge_wolfrum_180831_clusters(liger_k10_lambda5, 'RNA_snn_res.0.8')

plot_grid(
  UMAPPlot(liger_k10_lambda10, group.by='filtered_clusters', label=T),
  UMAPPlot(liger_k15_lambda10, group.by='filtered_clusters', label=T),
  UMAPPlot(liger_k10_lambda35, group.by='filtered_clusters', label=T),
  UMAPPlot(liger_k10_lambda5, group.by='filtered_clusters', label=T),
  ncol=2
)
```
...
```{r, fig.height = 25, fig.width = 25, fig.align = "center"}
harmony_180831 <- DimPlot(harmony, reduction='umap', group.by='branch_labels_integrated', cols=colormap, pt.size=0.5) + NoAxes()
harmony_180831_vf <- AugmentPlot(harmony_180831, width=25, height=25)
...
harmony_180831_split_by_source_vf <- AugmentPlot(harmony_180831_split_by_source, width=12, height=5)
#save_plot("../figures/figures_paper/main_figures/Figure_wolfrum/UMAP_wolfrum_harmony_branch-labels.split-dataset.vf.pdf", harmony_180831_split_by_source_vf, base_width=9, base_height=5)
...
harmony_180831_vf <- AugmentPlot(harmony_180831, width=7, height=5)
...
```{r, fig.height = 10, fig.width = 8.5, fig.align = "center"}
# g <- plot_grid(
#   VlnPlot(data.180831.brown.T4, features='EBF2', group.by='State.labels', pt.size=-1, cols=colors.states.labels) + NoLegend() + ggtitle('EBF2: Brown - T4'),
#   VlnPlot(data.180831.brown.T5, features='EBF2', group.by='State.labels', pt.size=-1, cols=colors.states.labels) + NoLegend() + ggtitle('EBF2: Brown - T5'),
#   VlnPlot(data.180831.brown.T4.peri, features='EBF2', group.by='State.labels', pt.size=-1, cols=colors.states.labels) + NoLegend() + ggtitle('EBF2: Peri - T4'),
#   VlnPlot(data.180831.brown.T5.peri, features='EBF2', group.by='State.labels', pt.size=-1, cols=colors.states.labels) + NoLegend() + ggtitle('EBF2: Peri - T5'),
#   VlnPlot(data.180831.brown.T4.supra, features='EBF2', group.by='State.labels', pt.size=-1, cols=colors.states.labels) + NoLegend() + ggtitle('EBF2: Supra - T4'),
#   VlnPlot(data.180831.brown.T5.supra, features='EBF2', group.by='State.labels', pt.size=-1, cols=colors.states.labels) + NoLegend() + ggtitle('EBF2: Supra - T5'),
#   ncol=2
# )
# 
# g
...
```{r, fig.height = 10, fig.width = 8.5, fig.align = "center"}
# g <- plot_grid(
#   VlnPlot(data.180831.brown, features='EBF2', group.by='branch_high_res', pt.size=-1, cols=colors.deciles.gradient) + NoLegend() + ggtitle('EBF2: Brown'),
#   VlnPlot(data.180831.peri, features='EBF2', group.by='branch_high_res', pt.size=-1, cols=colors.deciles.gradient) + NoLegend() + ggtitle('EBF2: Peri'),
#   VlnPlot(data.180831.supra, features='EBF2', group.by='branch_high_res', pt.size=-1, cols=colors.deciles.gradient) + NoLegend() + ggtitle('EBF2: Supra'),
#   ncol=1
# )
...
```{r}
#save_plot("figures/figures_paper/main_figures/Figure_wolfrum/ebf2_pseudotime_deciles.pdf", g, base_width=8.5, base_height=10)
```
