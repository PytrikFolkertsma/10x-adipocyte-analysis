...
##############################################
#       Finding brown sample markers         #
##############################################

samples <- as.character(data@meta.data$sample_name)
samples[which(data@meta.data$type == 'white')] <- 'white'
data@meta.data['test'] <- samples
data <- SetAllIdent(data, id='test')

brown_samples <- sort(unique(data@meta.data$sample_name[data@meta.data$depot %in% c('Supra', 'Peri')]))

print('Finding marker genes for brown samples:')
print(brown_samples)
...
clusterEvalQ(cl, library(tidyverse))
clusterEvalQ(cl, library(rlang)) # needed for using UQ()
list_of_dfs.all_markers <- parLapply(cl, brown_samples, function(x) FindMarkers(data, ident.1 = x, ident.2='white', min.pct = 0.1, test.use='negbinom'))
stopCluster(cl)

list_of_dfs.all_markers <- lapply(list_of_dfs.all_markers, function(x) cbind(x,gene=rownames(x))) # add gene name as column
names(list_of_dfs.all_markers) <- brown_samples # set names so bind_rows will get the .id correct
df.cluster_markers <- bind_rows(list_of_dfs.all_markers, .id="sample_name") #combine list of dfs into a single data frame

print('saving dataframe marker genes')
write.table(df.cluster_markers, file=paste(opt$outdir, 'markers_', output_prefix, '_brown', sep=''), sep='\t', row.names=F, quote=F)
